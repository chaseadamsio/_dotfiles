...
dotfilesLuaWatcher =
  hs.pathwatcher.new(os.getenv "HOME" .. "/src/github.com/chaseadamsio/dotfiles/", reloadConfig):start()
hs.notify.new({ title = "Hammerspoon", informativeText = "Reloaded Config" }):send()

function copyCurrChromeHighlight()
  local wasSuccessful, jsout =
    hs.osascript.javascript "Application('Google Chrome').windows[0].activeTab.execute({javascript:'window.getSelection().toString()'})"
